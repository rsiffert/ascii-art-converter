...
BitmapImage::BitmapImage(std::filesystem::path path) :
		full_img_(), bitmap_blocks_(), ascii_blocks_(), ref_ascii_blocks_(
				nullptr) {
...
void BitmapImage::Blockify() {
	...
	const uint16_t block_height_in_pix =
			ref_ascii_blocks_->front().HeightInPix();
	...
	for (uint64_t vert_block_idx = 0; vert_block_idx < num_vert_blocks;
		...
		for (uint64_t hor_block_idx = 0; hor_block_idx < num_hor_blocks;
			...
			ptrdiff_t right_pos_in_pix = (hor_block_idx + 1)
					* block_width_in_pix;
			ptrdiff_t bottom_pos_in_pix = (vert_block_idx + 1)
					* block_height_in_pix;

			BitmapBlock bmpBlock(
					full_img_.Region(left_pos_in_pix, top_pos_in_pix,
							right_pos_in_pix, bottom_pos_in_pix));

			bitmap_blocks_.insert(
					std::pair<BlockCoordinate, BitmapBlock>(
...
void BitmapImage::ConvertToAscii(
		const std::vector<AsciiBlock> &ref_ascii_blocks,
...
void BitmapImage::DumpAsciiBlocksAsTxt(
		const std::filesystem::path &output_filename) {
	...
	for (uint64_t row_block_idx = 0; row_block_idx < num_vert_blocks;
		...
		for (uint64_t hor_block_idx = 0; hor_block_idx < num_hor_blocks;
			...
			const char character = ascii_blocks_.at(
					BlockCoordinate(row_block_idx, hor_block_idx));
...
void BitmapImage::ConvertBlocksToAsciiBlocks() {
	for (auto block_it = bitmap_blocks_.begin();
			block_it != bitmap_blocks_.end(); block_it++) {
		const char closest_ascii_char = block_it->second.ClosestAsciiCharacter(
				*ref_ascii_blocks_);

		ascii_blocks_.insert(
				std::pair<BlockCoordinate, char>(block_it->first,
						closest_ascii_char));
	}
}
